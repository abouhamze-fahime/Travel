		...
		var yadcf = (function () {
			...
			function dateSelect(pDate, pEvent) {
				...
				if (pDate.namespace === 'daterangepicker') {
					...
					if ($(event).attr("id").indexOf("-from-") !== -1) {
						from = document.getElementById($(event).attr("id")).getAttribute("data-gdate");
						to = document.getElementById($(event).attr("id").replace("-from-", "-to-")).getAttribute("data-gdate");
					} else {
						to = document.getElementById($(event).attr("id")).getAttribute("data-gdate");
						from = document.getElementById($(event).attr("id").replace("-to-", "-from-")).getAttribute("data-gdate");
					}
				}
			...
			function addRangeDateFilter(filter_selector_string, table_selector_jq_friendly, column_number, filter_reset_button_text, filter_default_label, date_format) {
				...
				if (columnObj.datepicker_type === 'jquery-ui') {
					...
					$fromInput.persianDatepicker($.extend(datepickerObj, {
						onSelect: function () {
							dateSelect($fromInput.attr("data-gdate"), $fromInput);
						}
					}));
					$toInput.persianDatepicker($.extend(datepickerObj, {
						onSelect: function () {
							dateSelect($toInput.attr("data-gdate"), $toInput);
						}
					}));
